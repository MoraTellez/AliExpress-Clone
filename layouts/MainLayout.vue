  ...
  <div id="MainLayout" class="w-full fixed z-50">
    <div id="TopMenu" class="w-full bg-[#FAFAFA] border-b md:block hidden">
      <ul class="flex items-center justify-end text-xs text-[#333333] font-light px-2 h-10 bg-[#FAFAFA] max-w-[1200px] ">
        <li class="border-r border-r-gray-400 px-3 cursor-pointer hover:text-[#FF4646]">
          Sell on AliExpress
        </li>
        <li class="border-r border-r-gray-400 px-3 cursor-pointer hover:text-[#FF4646]">
          Cookie Preferences
        ...
        <li class="px-3 cursor-pointer hover:text-[#FF4646]">
          ...
          <div id="AccountMenu" v-if="isAccountMenu"
            ...
            <div v-if="!user">
              ...
              <div class="flex item-center gap-1 px-3 mb-3">
                <NuxtLink to="/auth"
                  class="bg-[#FF4646] text-center w-full text-[16px] rounded-sm text-white font-semibold p-2">Login /
                  Register</NuxtLink>
              </div>
            </div>
            <div class="border-b" />
            <ul class="bg-white">
              ...
              <li v-if="user" @click="client.auth.signOut()" class="text-[13px] py-2 px-4 w-full hover:bg-gray-200">Sing
                out</li>
            </ul>
          </div>
        </li>
    ...
    <div id="MainHeader" class="flex items-center w-full bg-white">
      <div class="flex lg:justify-start justify-between gap-10 max-w-[1150px] w-full px-3 py-5 mx-auto">
        <Nuxtlink to="/" class="min-w-[170px]">
          <img src="/AliExpress-logo.png" alt="logo" width="170">
        </Nuxtlink>
              ...
              <div v-if="items && items.data" v-for="item in items.data" class="p-1">
                <NuxtLink :to="`/item/${item.id}`"
                  ...
                  <div class="flex items-center">
                    <img :src="item.url" alt="" class="rounded-md" width="40">
        ...
        <NuxtLink to="/shoppingcart" class="flex items-center">
          <button class="relative md:block hidden" @mouseenter="isCartHover = true" @mouseleave="isCartHover = false">
            ...
            <div class="min-w-[40px]">
              <Icon size="33" name="ph:shopping-cart-simple-light" :color="isCartHover ? '#FF4646' : ''" />
            </div>
          </button>
        </NuxtLink>
