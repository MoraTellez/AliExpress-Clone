  ...
  <div :id="`ProductComponent${product.id}`"
    ...
    <NuxtLink :to="`/item/${product.id}`" >
      ...
      <div id="ProductDetails">
        ...
        <div class="flex items-center gap-1 relative px-1 -top-1">
        ...
        <p class="flex items-center px-1 pt-0.5 text-xs text-[#252525]">
          5,000+ sold <Icon color="#757575" class="ml-1.5" name="material-symbols:star-rate" /> 4.7
        </p>
        <p class="px-1 pt-0.5 text-xs text-[#252525]"> {{ product.title.substring(0, 60) }}</p>
        <p class="px-1 pb-1"><span class="text-[#009A66] text-xs font-semibold">Free shipping</span></p>
      </div>
    </NuxtLink>
  </div>
</template>
...
const oldPriceComputed = computed(() => {
  let res = (product.value.price + (product.value.price / 200)) / 100
